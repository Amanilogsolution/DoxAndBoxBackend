...
const ShreddingDupliacte = async (req, res) => { 
    ...
    // console.log(`select fileno From TBL_INWARDFILESCAN where fileno in ('${files.map(data => data.file_number).join("','")}') and CUSTID='${CUSTID}' and WH='${WH}'`)
    try{
        ...
        console.log(result.recordset.map(item => item))
    }
    catch(err){
        res.send(err)
        }
